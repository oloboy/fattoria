    ...
    <nav className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-5xl rounded-[2rem] px-6 py-4 transition-all duration-500 border ${scrolled ? 'bg-cream/80 backdrop-blur-xl border-moss/10 text-charcoal' : 'bg-transparent border-transparent text-cream'}`}>
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 font-medium text-sm">
          <a href="#prodotti" className="link-hover">I Liquori</a>
          <a href="#filosofia" className="link-hover">Filosofia</a>
          <a href="#bomboniere" className="link-hover">Bomboniere & Eventi</a>
        </div>
        ...
        <div className="hidden md:block">
          <a href="#contatti" className="btn-magnetic bg-moss text-cream px-6 py-2.5 rounded-[1.5rem] font-medium text-sm inline-block border border-moss-light">
            <span className="relative z-10 transition-colors duration-300">Contattaci</span>
      ...
      <div className={`md:hidden absolute top-full left-0 right-0 mt-4 bg-cream/95 backdrop-blur-xl rounded-[2rem] border border-moss/10 p-6 flex-col gap-4 transition-all duration-300 ${isOpen ? 'flex opacity-100 translate-y-0 pointer-events-auto' : 'hidden opacity-0 -translate-y-4 pointer-events-none'}`}>
        <a href="#prodotti" className="text-charcoal font-medium text-lg" onClick={() => setIsOpen(false)}>I Liquori</a>
        <a href="#filosofia" className="text-charcoal font-medium text-lg" onClick={() => setIsOpen(false)}>Filosofia</a>
        <a href="#bomboniere" className="text-charcoal font-medium text-lg" onClick={() => setIsOpen(false)}>Bomboniere & Eventi</a>
        <a href="#contatti" className="bg-clay text-cream px-6 py-3 rounded-[1.5rem] font-medium text-center mt-2" onClick={() => setIsOpen(false)}>Contattaci</a>
      </div>
    </nav>
  );
}
